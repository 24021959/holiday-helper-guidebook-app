...
export const AdminProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchPages = async () => {
    ...
    try {
      // Check if we're in demo mode
      if (isDemoMode || localStorage.getItem("admin_token") === "demo_token" || 
          localStorage.getItem("admin_token") === "master_token") {
        console.log("Using demo pages data");
        ...
        const demoPages: PageData[] = [
          {
            id: "1",
            title: "Home",
            content: "<p>Benvenuto al nostro sito!</p>",
            path: "home",
            icon: "Home",
            isSubmenu: false,
            pageImages: [],
          ...
          },
          {
            id: "2",
            title: "Chi Siamo",
            content: "<p>Informazioni sulla nostra azienda</p>",
            path: "about",
            icon: "Info",
            isSubmenu: false,
            pageImages: [],
          ...
          },
          {
            id: "3",
            title: "Contatti",
            content: "<p>Contattaci per maggiori informazioni</p>",
            path: "contact",
            icon: "Phone",
            isSubmenu: false,
            pageImages: [],
      ...
      // If not in demo mode, try to fetch from database
      try {
        ...
        if (error) {
          ...
          // Use fallback demo data in case of error
          const demoPages: PageData[] = [
            {
              id: "1",
              title: "Home",
  ...
  const fetchKeywordToIconMap = async () => {
    try {
      // If we're in demo mode, use default map
      if (isDemoMode || localStorage.getItem("admin_token") === "demo_token" || 
          localStorage.getItem("admin_token") === "master_token") {
